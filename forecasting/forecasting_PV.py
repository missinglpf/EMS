...
def read_data_load(train_start,train_end,test_start,test_end):
	...
	train_data = ems_session.query(ForecastDBData.PV_PG).\
		filter(and_(ForecastDBData.TIME_STAMP>=train_start,ForecastDBData.TIME_STAMP<=train_end)).all()
	print('..................................................train data')
	print(train_data)
	train_len = len(train_data)

	test_data = ems_session.query(ForecastDBData.PV_PG). \
...
def series_to_supervised(data, n_in=1, n_out=1, dropnan=True):
    ...
    cols.append(df.shift(48))
    names += [('var%d(t-%d)' % (j + 1, 48)) for j in range(n_vars)]
    cols.append(df.shift(36))
    names += [('var%d(t-%d)' % (j + 1, 36)) for j in range(n_vars)]
    cols.append(df.shift(12))
    names += [('var%d(t-%d)' % (j + 1, 12)) for j in range(n_vars)]
    cols.append(df.shift(6))
    names += [('var%d(t-%d)' % (j + 1, 6)) for j in range(n_vars)]
    cols.append(df.shift(5))
    names += [('var%d(t-%d)' % (j + 1, 5)) for j in range(n_vars)]
    cols.append(df.shift(4))
    names += [('var%d(t-%d)' % (j + 1, 4)) for j in range(n_vars)]
    cols.append(df.shift(3))
    names += [('var%d(t-%d)' % (j + 1, 3)) for j in range(n_vars)]
...
def forecastingPV(testtime, PVmodel):
    test1 = testtime
    ...
    for i in range(0, len(forecasts)):
        ...
        if forecasts[i][0]<0:
            forecasts[i][0] = 0.000001
        row1.PV_PG_f = forecasts[i][0]
        row2 = ems_session.query(ForecastDBData).filter_by(TIME_STAMP=selected_dates[i + 1][0]).first()
        if forecasts[i][1]<0:
            forecasts[i][1] = 0.000001
        row2.PV_PG_f2 = forecasts[i][1]
        print('i')
        print(forecasts[i][0])
        print('>>>')
    ems_session.commit()
...
# forecastingPV(testtime, PVmodel)